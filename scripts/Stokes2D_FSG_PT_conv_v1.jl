# VISCOUS INCLUSION PURE SHEAR TEST + Vol. Harm Haveragin of Mozco et al. 2002
# Initialisation
...
@views function Stokes2S_FSG()
    ...
    nxm, nym = 4*n, 4*n
    xm1 = LinRange(xmin+Δx/8, xmax-Δx/8, ncx*nxm)
    ym1 = LinRange(ymin+Δy/8, ymax-Δy/8, ncy*nym)
    (xm2, ym2) = ([x for x=xm1,y=ym1], [y for x=xm1,y=ym1])
    xm, ym = xm2[:], ym2[:]
    tm = ones(Int,length(xm))
    tm[ (xm.-x0).^2 .+ (ym.-y0).^2 .< rad^2 ] .= 2

    N_1  =    zeros(Dat, 2, ncx-0, ncy+1)
    N_2  =    zeros(Dat, 2, ncx+1, ncy+0)
    Nt_1 =    zeros(Dat, ncx-0, ncy+1) 
    Nt_2 =    zeros(Dat, ncx+1, ncy+0)
    for i in eachindex(xm)
       ix = Int(floor((xm[i] )/Δx)) + 1 
       iy = Int(floor((ym[i] + Δy/2)/Δy)) + 1 
       N_1[tm[i],ix,iy] += 1
       Nt_1[ix,iy] += 1

       ix = Int(floor((xm[i] + Δx/2)/Δx)) + 1 
       iy = Int(floor((ym[i]       )/Δy)) + 1 
       N_2[tm[i],ix,iy] += 1
       Nt_2[ix,iy] += 1
    end
    for i=1:2
    ...
    # vol_1       =   zeros(Dat, ncx-0, ncy+1)
    # x_1, y_1    = LinRange(xmin, xmax, ncx+1), LinRange(ymin-Δy/2, ymax+Δy/2, ncy+2)
    # in  = 0
    # for j=1:ncy+1
    #     for i=1:ncx
    #         sw = x_1[i]^2 + y_1[j]^2 < rad^2 
    #         se = x_1[i+1]^2 + y_1[j]^2 < rad^2
    #         nw = x_1[i]^2 + y_1[j+1]^2 < rad^2 
    #         ne = x_1[i+1]^2 + y_1[j+1]^2 < rad^2
    #         if sw || se || nw || ne
    #             # intersection to west?
    #             xW = x_1[i] - x0
    #             yW = sqrt(abs(rad^2 - xW^2)) + y0
    #             iW = false
    #             if yW>y_1[j] && yW<y_1[j+1] iW=true end
    #             # intersection to east?
    #             xE = x_1[i+1] - x0
    #             yE = sqrt(abs(rad^2 - xE^2)) + y0
    #             iE = false
    #             if yE>y_1[j] && yE<y_1[j+1] iE=true end
    #             # intersection to south?
    #             yS = y_1[j] - y0
    #             xS = sqrt(abs(rad^2 - yS^2)) + x0
    #             iS = false
    #             if xS>x_1[i] && xS<x_1[i+1] iS=true end
    #             # intersection to north?
    #             yN = y_1[j+1] - y0
    #             xN = sqrt(abs(rad^2 - yN^2)) + x0
    #             iN = false
    #             if xN>x_1[i] && xN<x_1[i+1] iN=true end
    #             println("case")
    #             println(iW, ' ', iE, ' ', iS, ' ', iN)
    #             println(sw, ' ', se, ' ', nw, ' ', ne)
    #         end
    ...
    for i in eachindex(Vx_1)
        Vx, Vy = SolutionFields_v(1.0, 1.00, rad, εbg, 0.0, xv2_1[i]-x0, yv2_1[i]-y0, 0)
        Vx_1[i] = Vx
        Vy_1[i] = Vy
    end
    ...
    for i in eachindex(P_1_ana)
        Vx, Vy, P = EvalAnalDani( xc2_1[i]-x0, yc2_1[i]-y0, rad, 1.0, 100.0 )
        P_1_ana[i] = P
    end
    P_2_ana =   zeros(Dat, ncx+1, ncy-0)
    for i in eachindex(P_2_ana)
        Vx, Vy, P = EvalAnalDani( xc2_2[i]-x0, yc2_2[i]-y0, rad, 1.0, 100.0 )
        P_2_ana[i] = P
    end
    ...
    # Viscosity usin volume harmonic mean
    η_1 .= (N_1[1,:,:]./1.0 .+ N_1[2,:,:]./100).^(-1)
    η_2 .= (N_2[1,:,:]./1.0 .+ N_2[2,:,:]./100).^(-1)
    # # η_1  .= 1.0; η_2  .= 1.0
    ...
    # # Plotting
    # p1 = heatmap( xc_1, yc_1, N_1[1,:,:]', aspect_ratio=1, xlims=(xmin, xmax), ylims=(ymin, ymax), c=:turbo, title="Vx_1")
    # p2 = heatmap( xc_1, yc_1, N_1[2,:,:]', aspect_ratio=1, xlims=(xmin, xmax), ylims=(ymin, ymax), c=:turbo, title="Vy_1")
    # p3 = heatmap( xc_2, yc_2, N_2[1,:,:]', aspect_ratio=1, xlims=(xmin, xmax), ylims=(ymin, ymax), c=:turbo, title="P_1")
    # p4 = heatmap( xc_2, yc_2, N_2[2,:,:]', aspect_ratio=1, xlims=(xmin, xmax), ylims=(ymin, ymax), c=:turbo, title="P_2")

    p1 = heatmap( xv_1, yv_1, Vx_1', aspect_ratio=1, xlims=(xmin, xmax), ylims=(ymin, ymax), c=:turbo, title="Vx_1")
    p2 = heatmap( xv_1, yv_1, Vy_1', aspect_ratio=1, xlims=(xmin, xmax), ylims=(ymin, ymax), c=:turbo, title="Vy_1")
    p3 = heatmap( xc_1, yc_1, P_1', aspect_ratio=1, xlims=(xmin, xmax), ylims=(ymin, ymax), c=:turbo, title="P_1")
    p4 = heatmap( xc_1, yc_1, abs.(P_1.-P_1_ana)', aspect_ratio=1, xlims=(xmin, xmax), ylims=(ymin, ymax), c=:turbo, title="P_2")

#     y_int = h.(xv, slope, ord)
#     p4 = plot(xv, y_int, aspect_ratio=1, xlims=(xmin, xmax), ylims=(ymin, ymax), color=:black, label=:none)
#     ied1  = [ 1 2 3 4] 
#     ied2  = [ 2 3 4 1] 
#     for ic=1:length(XC)
#         for ied=1:4
...
#     #     p4 = plot!(xv, y_line, aspect_ratio=1, xlims=(xmin, xmax), ylims=(ymin, ymax), color=:gray, label=:none)
#     # end
#     # for i=1:ncx+1
#     #     x_line = xv[i]*ones(size(yv))
#     #     p4 = plot!(x_line, yv, aspect_ratio=1, xlims=(xmin, xmax), ylims=(ymin, ymax), color=:gray, label=:none)
#     # end
