...
function main(::Type{DAT}; device) where DAT
    ...
    ε̇bg        = -1*0
    rad        = 0.5
    ϵ          = 1e-4      # nonlinear tolerence
    it         = 1
    itmax      = 5000     # max number of iters
    ...
    ########### Buoyancy
    ρ[xv4.^2 .+ yv4.^2 .< rad^2] .= ρ_inc
    b.y.v   .= to_device( ρ[2:2:end-1, 2:2:end-1]*g )
    b.y.c   .= to_device( ρ[1:2:end-0, 1:2:end-0]*g )
