...
function main(::Type{DAT}; device) where DAT
    ...
    iterMax  = 5e9     # max number of iters
    nout     = 1e3       # check frequency
    Reopt    = 0.625*π*2
    cfl      = 1.0/1.3
    ρ        = cfl*Reopt/ncx
    nsm      = 4
    ...
    d1 = ones(Float32, ncx+1, ncy)
    d2 = ones(Float32, ncx+1, ncy)
    d1 .= 2.0 .* to_host(N.x.x).^2 .* to_host(N.y.x).^2
    d2 .= to_host(N.x.x) .* to_host(N.y.x) .* (-to_host(N.x.x).^2 .+ to_host(N.y.x).^2)

    D.d11.x .= to_device(2*ηx .-2*(1 .- 1/ani_fac).*d1.*ηx)
    D.d12.x .= to_device(       2*(1 .- 1/ani_fac).*d1.*ηx)
    D.d13.x .= to_device(      -4*(1 .- 1/ani_fac).*d2.*ηx)
    D.d14.x .= to_device(      -4*(1 .- 1/ani_fac).*d2.*ηx)
    D.d21.x .= to_device(       2*(1 .- 1/ani_fac).*d1.*ηx)
    D.d22.x .= to_device(2*ηx .-2*(1 .- 1/ani_fac).*d1.*ηx)
    D.d23.x .= to_device(       4*(1 .- 1/ani_fac).*d2.*ηx)
    D.d24.x .= to_device(       4*(1 .- 1/ani_fac).*d2.*ηx)
    D.d31.x .= to_device(      -2*(1 .- 1/ani_fac).*d2.*ηx)
    D.d32.x .= to_device(       2*(1 .- 1/ani_fac).*d2.*ηx)
    D.d33.x .= to_device(2*ηx .+4*(1 .- 1/ani_fac).*(d1 .- 0.5).*ηx)
    D.d34.x .= to_device(       4*(1 .- 1/ani_fac).*(d1 .- 0.5).*ηx)
    D.d41.x .= to_device(      -2*(1 .- 1/ani_fac).*d2.*ηx)
    D.d42.x .= to_device(       2*(1 .- 1/ani_fac).*d2.*ηx)
    D.d43.x .= to_device(       4*(1 .- 1/ani_fac).*(d1 .- 0.5).*ηx)
    D.d44.x .= to_device(2*ηx .+4*(1 .- 1/ani_fac).*(d1 .- 0.5).*ηx)
    
    d1 = ones(Float32, ncx, ncy+1)
    d2 = ones(Float32, ncx, ncy+1)
    d1 .= 2.0 .* to_host(N.x.y).^2 .* to_host(N.y.y).^2
    d2 .= to_host(N.x.y) .* to_host(N.y.y) .* (-to_host(N.x.y).^2 .+ to_host(N.y.y).^2)

    D.d11.y .= to_device(2*ηy .-2*(1 .- 1/ani_fac).*d1.*ηy)
    D.d12.y .= to_device(       2*(1 .- 1/ani_fac).*d1.*ηy)
    D.d13.y .= to_device(      -4*(1 .- 1/ani_fac).*d2.*ηy)
    D.d14.y .= to_device(      -4*(1 .- 1/ani_fac).*d2.*ηy)
    D.d21.y .= to_device(       2*(1 .- 1/ani_fac).*d1.*ηy)
    D.d22.y .= to_device(2*ηy .-2*(1 .- 1/ani_fac).*d1.*ηy)
    D.d23.y .= to_device(       4*(1 .- 1/ani_fac).*d2.*ηy)
    D.d24.y .= to_device(       4*(1 .- 1/ani_fac).*d2.*ηy)
    D.d31.y .= to_device(      -2*(1 .- 1/ani_fac).*d2.*ηy)
    D.d32.y .= to_device(       2*(1 .- 1/ani_fac).*d2.*ηy)
    D.d33.y .= to_device(2*ηy .+4*(1 .- 1/ani_fac).*(d1 .- 0.5).*ηy)
    D.d34.y .= to_device(       4*(1 .- 1/ani_fac).*(d1 .- 0.5).*ηy)
    D.d41.y .= to_device(      -2*(1 .- 1/ani_fac).*d2.*ηy)
    D.d42.y .= to_device(       2*(1 .- 1/ani_fac).*d2.*ηy)
    D.d43.y .= to_device(       4*(1 .- 1/ani_fac).*(d1 .- 0.5).*ηy)
    D.d44.y .= to_device(2*ηy .+4*(1 .- 1/ani_fac).*(d1 .- 0.5).*ηy)
    ...
    ΔτP      = 1*cfl*minimum(ηx/ani_fac)*min(Δ...)/2
