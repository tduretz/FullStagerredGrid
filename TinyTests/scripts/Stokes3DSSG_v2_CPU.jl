...
function   maxloc(A, i, j, k)  
    return max( max( max( max(A[i-1,j  ,k  ], A[i+1,j  ,k  ])  , A[i  ,j  ,k  ] ),
           max(A[i  ,j-1,k  ], A[i  ,j+1,k  ]) ),
           max(A[i  ,j  ,k-1], A[i  ,j  ,k+1]) )  
end # Modified from ParallelStencil.jl - https://github.com/omlins/ParallelStencil.jl/blob/main/src/FiniteDifferences.jl

###############################

function ApplyMaxLoc!( ηv2, ηv )
    Threads.@threads for k = 2:size(ηv2,3)-1
        @inbounds for j = 2:size(ηv2,2)-1
            for i = 2:size(ηv2,1)-1
                ηv2[i,j,k] = maxloc(ηv,i,j,k)
            end
        end
    end
    return nothing
...
function Stokes3D(n, ::Type{DAT}; device) where DAT
    ...
    vtk_grid("Output", xce[2:end-1], yce[2:end-1], zce[2:end-1]) do vtk
        vtk["Pressure"] = P[2:end-1,2:end-1,2:end-1]
    end
