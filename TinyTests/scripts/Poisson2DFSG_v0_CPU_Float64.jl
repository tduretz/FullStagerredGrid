...
using TinyKernels, Printf, GLMakie, Makie.GeometryBasics, MAT, JLD2
...
@tiny function SolutionUpdatePoisson!(T, ∂T∂τ, ΔτV)
    i, j = @indices

    @inbounds if i>1 && j>1 && i<size(T.v,1) && j<size(T.v,2)
        T.v[i,j] += ΔτV.v[i,j] * ∂T∂τ.v[i,j]
    end
    @inbounds if i>1 && j>1 && i<size(T.c,1) && j<size(T.c,2)
        T.c[i,j] += ΔτV.c[i,j] * ∂T∂τ.c[i,j]
    end
end

@tiny function RateUpdatePoisson!(∂T∂τ, R, θ)
    i, j = @indices

    @inbounds if i>1 && j>1 && i<size(R.c,1) && j<size(R.c,2)
        ∂T∂τ.c[i,j] = (1-θ) * ∂T∂τ.c[i,j] + R.c[i,j]
    end

    @inbounds if i>1 && j>1 && i<size(R.v,1) && j<size(R.v,2)
        ∂T∂τ.v[i,j] = (1-θ) * ∂T∂τ.v[i,j] + R.v[i,j]
    end
end


@tiny function ResidualPoisson!(R, qx, qy, ∂, Δ)
    i, j = @indices
    @inline isin(A) = checkbounds(Bool, A, i, j)

     if isin(R.v) && j>1 && j<size(R.v,2) && i>1 && i<size(R.v,1)
        ∂ξ∂x = ∂.ξ.∂x.v[i,j]; ∂ξ∂y = ∂.ξ.∂y.v[i,j]; ∂η∂x = ∂.η.∂x.v[i,j]; ∂η∂y = ∂.η.∂y.v[i,j]
        
        ∂qx∂ξ     = ∂_∂ξ(qx.y[i+1,j], qx.y[i,j], Δ)
        ∂qy∂ξ     = ∂_∂ξ(qy.y[i+1,j], qy.y[i,j], Δ)
    
        ∂qx∂η     = ∂_∂η(qx.x[i,j+1], qx.x[i,j], Δ)
        ∂qy∂η     = ∂_∂η(qy.x[i,j+1], qy.x[i,j], Δ)

        ∂qx∂x     = ∂qx∂ξ * ∂ξ∂x + ∂qx∂η * ∂η∂x
        ∂qy∂y     = ∂qy∂ξ * ∂ξ∂y + ∂qy∂η * ∂η∂y 
    ...
    @inbounds if i>1 && j>1 && i<size(R.c,1) && j<size(R.c,2)
        ...
        ∂qx∂ξ     = ∂_∂ξ(qx.x[i,j-1+1], qx.x[i-1,j-1+1], Δ)
        ∂qy∂ξ     = ∂_∂ξ(qy.x[i,j-1+1], qy.x[i-1,j-1+1], Δ)
    
        ∂qx∂η     = ∂_∂η(qx.y[i-1+1,j], qx.y[i-1+1,j-1], Δ)
        ∂qy∂η     = ∂_∂η(qy.y[i-1+1,j], qy.y[i-1+1,j-1], Δ)
...
@tiny function HeatFlux!(qx, qy, T, ∂, Δ)
    i, j = @indices
    @inline isin(A) = checkbounds(Bool, A, i, j)
    if isin(qx.x) && j>1 && j<size(qx.x,2) #&& i>1 && i<size(ε̇.xx.x,1) # FREE SLIP W/E
        ∂ξ∂x = ∂.ξ.∂x.x[i,j]; ∂ξ∂y = ∂.ξ.∂y.x[i,j]; ∂η∂x = ∂.η.∂x.x[i,j]; ∂η∂y = ∂.η.∂y.x[i,j];
        ∂T∂ξ      = ∂_∂ξ(T.c[i+1,j+1-1], T.c[i+0,j+1-1], Δ) 
        ∂T∂η      = ∂_∂η(T.v[i+0,j+1-1], T.v[i+0,j+0-1], Δ)
        ∂T∂x      = ∂T∂ξ * ∂ξ∂x + ∂T∂η * ∂η∂x
        ∂T∂y      = ∂T∂ξ * ∂ξ∂y + ∂T∂η * ∂η∂y
        qx.x[i,j] = -∂T∂x
        qy.x[i,j] = -∂T∂y
    end
     if isin(qx.y) && i>1 && i<size(qx.y,1)
        ∂ξ∂x = ∂.ξ.∂x.y[i,j]; ∂ξ∂y = ∂.ξ.∂y.y[i,j]; ∂η∂x = ∂.η.∂x.y[i,j]; ∂η∂y = ∂.η.∂y.y[i,j];
        ∂T∂ξ       = ∂_∂ξ(T.v[i+1-1,j+0], T.v[i+0-1,j+0], Δ)
        ∂T∂η       = ∂_∂η(T.c[i+1-1,j+1], T.c[i+1-1,j+0], Δ)
        ∂T∂x       = ∂T∂ξ * ∂ξ∂x + ∂T∂η * ∂η∂x
        ∂T∂y       = ∂T∂ξ * ∂ξ∂y + ∂T∂η * ∂η∂y
        qx.y[i,j] = -∂T∂x
        qy.y[i,j] = -∂T∂y
...
function main(::Type{DAT}; device) where DAT
    ...
    T    = ScalarFSG(DAT, device, :CV, ncx, ncy)
    qx   = ScalarFSG(DAT, device, :XY, ncx, ncy)
    qy   = ScalarFSG(DAT, device, :XY, ncx, ncy)
    RT   = ScalarFSG(DAT, device, :CV, ncx, ncy)
    ∂T∂τ = ScalarFSG(DAT, device, :CV, ncx, ncy)
    ...
    kernel_HeatFlux!              = HeatFlux!(device)
    kernel_ResidualPoisson!       = ResidualPoisson!(device)
    kernel_RateUpdatePoisson!     = RateUpdatePoisson!(device)
    kernel_SolutionUpdatePoisson! = SolutionUpdatePoisson!(device)

    TinyKernels.device_synchronize(get_device())

    T.v .=  to_device( abs.(X.v.x .* ( X.v.y .+ 0.45)) )
    T.c .=  to_device( abs.(X.c.x .* ( X.c.y .+ 0.45)) )
    ...
    for it=it0+1:nt
        ...
        # Iterations
        for iter=1:1#5000#itmax
            wait( kernel_HeatFlux!(qx, qy, T, ∂, Δ; ndrange=(ncx+2,ncy+2) ) )
            wait( kernel_ResidualPoisson!(RT, qx, qy, ∂, Δ; ndrange=(ncx+2,ncy+2) ) )
            wait( kernel_RateUpdatePoisson!(∂T∂τ, RT, θ; ndrange=(ncx+2,ncy+2)))
            wait( kernel_SolutionUpdatePoisson!(T, ∂T∂τ, ΔτV; ndrange=(ncx+2,ncy+2)) )
            ...
            if iter==1 || mod(iter,nout)==0
                ...
                errt = (; c = mean(abs.(RT.c)), v = mean(abs.(RT.v)) ) 
                if iter==1 errt0 = errt end
                ...
                norm_Rt = 0.5*( norm( RT.c[2:end-1,2:end-1])/sqrt(length(RT.c[2:end-1,2:end-1]))  + norm(RT.v) /sqrt(length(RT.v)) )
                norm_Rp = 0.5*( norm(∇v.x[:,2:end-1])/sqrt(length(∇v.x[:,2:end-1])) + norm(∇v.y[2:end-1,:])/sqrt(length(∇v.y[2:end-1,:])) )
                @printf("it = %03d, iter = %05d, nRt=%1.6e, nRx=%1.6e nRy=%1.6e nRp=%1.6e\n", it, iter, norm_Rt, norm_Rx, norm_Ry, norm_Rp)
                if isnan(maximum(errt)) error("NaN à l'ail!") end
                if (maximum(errt)>1e3) error("Stop before explosion!") end
                if maximum(errt) < ϵ && maximum(erry) < ϵ && maximum(errp) < ϵ
                    break
                end
            end
        end
        ...
        ax2 = Axis(f[2, 1], title = "T.c", xlabel = "x [km]", ylabel = "y [km]")
        field = RT.c[2:end-1,2:end-1][:]
        min_v = minimum( field); max_v = maximum( field ) + 0.001
        limits = min_v ≈ max_v ? (min_v, min_v + 1) : (min_v, max_v)
        p = [Polygon( Point2f0[ (cell_vertx[i,j], cell_verty[i,j]) for j=1:4] ) for i in 1:length(sol.vx)]
        poly!(ax2, p, color = field, colormap = :turbo, strokewidth = 0, strokecolor = :white, markerstrokewidth = 0, markerstrokecolor = (0, 0, 0, 0), aspect=:image, colorrange=limits)
        Colorbar(f[2, 2], colormap = :turbo, limits=limits, flipaxis = true, size = 25 )
        yh0 .= Amp.*exp.(-(xh0.-x0).^2 ./ σ^2) 
        lines!(ax2, xh0, yh0)

        ax2 = Axis(f[2, 3], title = "T.v", xlabel = "x [km]", ylabel = "y [km]")
        field = RT.v[:]
        min_v = minimum( field ); max_v = maximum( field )
        limits = min_v ≈ max_v ? (min_v, min_v + 1) : (min_v, max_v)
        p = [Polygon( Point2f0[ (node_vertx[i,j], node_verty[i,j]) for j=1:4] ) for i in 1:length(field)]
        poly!(ax2, p, color = field, colormap = :turbo, strokewidth = 0, strokecolor = :white, markerstrokewidth = 0, markerstrokecolor = (0, 0, 0, 0), aspect=:image, colorrange=limits)
        ...
        @save "PoissonFSE_40x40.jld2" xn1=xv4 yn1=yv4 ∂1=∂ ∂x1=∂x ∂y1=∂y
        @show ΔτV.v
        @show θ
    end
end
