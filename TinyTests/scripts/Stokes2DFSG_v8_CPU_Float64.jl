...
# HMMM MESH DEFORMATION --> CRAZY JACOBIAN
...
# @tiny function DensityGradient!(ε̇, ∇v, V, P, η, ∂, Δ, options, fsp)
#     i, j = @indices
...
#         ∂ρ∂ξ  = ∂_∂ξ(V.x.c[i+1,j+1-1], V.x.c[i+0,j+1-1], Δ)
#         ∂ρ∂η  = ∂_∂η(V.y.v[i+0,j+1-1], V.y.v[i+0,j+0-1], Δ)
#         ∂ρ.x.c[i,j] = ∂ρ∂ξ * ∂ξ∂x + ∂ρ∂η * ∂η∂x
#         ∂ρ.y.v[i,j] = ∂ρ∂ξ * ∂ξ∂y + ∂ρ∂η * ∂η∂y
#     end
# end
...
function main(::Type{DAT}; device) where DAT
    ...
    if adapt_mesh
        ...
        X_msh  = zeros(2)
        if load_mesh 
            file = matopen(string(@__DIR__, "/DeformedMesh.mat"))
            xv4 = read(file, "xv4") 
            yv4 = read(file, "yv4") 
            close(file)
        else
        ...
        # Compute slope
        if options.topo 
            hxnum = -dhdx_num(xv4, yv4, Δ)
        end
        # # Compute forward transformation
        ∂x     = (∂ξ=zeros(size(yv4)), ∂η = zeros(size(yv4)) )
        ∂y     = (∂ξ=zeros(size(yv4)), ∂η = zeros(size(yv4)) )
        # ComputeForwardTransformation_ini!( ∂x, ∂y, X0.ξ, X0.η, X_msh, Amp, x0, σ, m, xmin, xmax, ymin, ymax, σx, σy, ϵjac, options)
    ...
    for it=1:nt
        ...
        if adapt_mesh
            # Advect
            @show Δt  = min(Δ...)/maximum(V.y.c)/2.1
            Vx4 = zero(xv4)
            Vx4[2:2:end-1, 2:2:end-1] .= V.x.v 
            Vx4[1:2:end-0, 1:2:end-0] .= V.x.c
            Vx4[2:2:end-1, 3:2:end-2] .= 0.5*(V.x.v[:,2:end] .+ V.x.v[:,1:end-1])
            Vx4[3:2:end-2, 2:2:end-1] .= 0.5*(V.x.v[2:end,:] .+ V.x.v[1:end-1,:])
            Vy4 = zero(yv4)
            ...
            file = matopen(string(@__DIR__, "/DeformedMesh.mat"), "w")
            write(file, "xv4", xv4)
            write(file, "yv4", yv4)
            close(file)
            ...
            ∂y.∂η[:,2:2:end-1] .= (yv4[:,3:2:end-0] .- yv4[:,1:2:end-2])./Δ.η
            ∂y.∂η[:,3:2:end-2] .= (yv4[:,4:2:end-1] .- yv4[:,2:2:end-3])./Δ.η
            ∂y.∂η[:,[1 end]]   .= ∂y.∂η[:,[2 end-1]]             # Solve for inverse transformation
            ∂ξ = (∂x=zeros(size(yv4)), ∂y=zeros(size(yv4))); ∂η = (∂x=zeros(size(yv4)), ∂y=zeros(size(yv4)))
            InverseJacobian!(∂ξ,∂η,∂x,∂y)
            ∂.ξ.∂x.x .= to_device( ∂ξ.∂x[2:2:end-1,1:2:end-0])
            ∂.ξ.∂x.y .= to_device( ∂ξ.∂x[1:2:end-0,2:2:end-1])
        ...
        yh0 .= h.(xh0,Amp,σ,0.,x0)
        lines!(ax2, xh0, yh0)

        ax2 = Axis(f[2, 3], title = "Vy", xlabel = "x [km]", ylabel = "y [km]")
        min_v = minimum( sol.vy ); max_v = maximum( sol.vy )
        limits = min_v ≈ max_v ? (min_v, min_v + 1) : (min_v, max_v)
        p = [Polygon( Point2f0[ (vertx[i,j], verty[i,j]) for j=1:4] ) for i in 1:length(sol.vy)]
        ...
        # ax = Axis(f[3, 1:2], title = "h_x", xlabel = "x [km]", ylabel = "y [km]")
        # lines!(ax, xv4[:,end-1], hxnum)
