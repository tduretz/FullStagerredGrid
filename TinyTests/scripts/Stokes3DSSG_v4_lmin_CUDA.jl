...
using TinyKernels, Printf, WriteVTK #, GLMakie, Makie.GeometryBasics, MAT
...
@tiny function Kernel_MaxLoc!(A, B)
    i, j, k = @indices
     if i>1 && j>1 && k>1 && i<size(A,1) && j<size(A,2) && k<size(A,3)
...
@tiny function Kernel_InitialCondition!( Vx, Vy, Vz, ηv, ηc, Gv, Gc, ε, xv, yv, zv, xce, yce, zce, rc )
    i, j, k = @indices
     if i<length(xce) && k<length(zce) && j<length(yce)
        ...
        if (i<=size(ηv,1) && j<=size(ηv,2) && k<=size(ηv,3)) 
            ...
            z = zv[k] - (-0.8150/2) 
            r = rc
            if ( (x*x + y*y + z*z) < r*r )  Gv[i,j,k] = 0.25 end  
        end
    end
    return
end
    ...
    for it=1:nt
        ...
        for iter=1:niter
            ...
            if mod(iter,nout) == 0 || iter==1
                ...
                @printf("Step %04d --- Iter. %05d --- %d * %d * %d\n", it, iter, ncx, ncy, ncz) 
                @printf("Fx = %1.4e\n", nFx) 
                @printf("Fy = %1.4e\n", nFy) 
                @printf("Fz = %1.4e\n", nFz) 
                @printf("Fp = %1.4e\n", nFp) 
...
@time Stokes3D( 4, eletype; device )
