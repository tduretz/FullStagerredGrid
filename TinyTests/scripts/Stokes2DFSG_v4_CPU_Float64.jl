...
@tiny function StrainRates!(ε̇, ∇v, V, P, η, ∂, Δ, options)
    ...
    @inbounds if isin(∇v.y)
        ...
        if options.free_surface && j == size(∇v.y, 2)
            ∂Vx∂η  = - ∂Vy∂ξ
...
@tiny function Residuals!(R, τ, P, ∂, b, Δ, options, ε̇, η)
    i, j = @indices
    ...
    @inbounds if i>1 && j>1 && i<size(R.x.v,1) && j<North
        ...
        if j<size(R.x.v,2)
            ∂τxx∂η     = ∂_∂η(τ.xx.x[i,j], τ.xx.x[i,j-1], Δ)
            ∂τyy∂η     = ∂_∂η(τ.yy.x[i,j], τ.yy.x[i,j-1], Δ)
            ∂τyx∂η     = ∂_∂η(τ.xy.x[i,j], τ.xy.x[i,j-1], Δ)
            ∂P∂η       = ∂_∂η(   P.x[i,j],    P.x[i,j-1], Δ)
        else
            ∂τxx∂η     = ∂_∂η(0., τ.xx.x[i,j-1], Δ)
            ∂τyy∂η     = ∂_∂η(0., τ.yy.x[i,j-1], Δ)
            ∂τyx∂η     = ∂_∂η(0., τ.xy.x[i,j-1], Δ)
            ∂P∂η       = ∂_∂η(0.,    P.x[i,j-1], Δ)
        end
...
function main(::Type{DAT}; device) where DAT
    ...
    τ    = TensorFSG_old(DAT, device, :XY, ncx, ncy)
    ε̇    = TensorFSG_old(DAT, device, :XY, ncx, ncy)
    ...
    # kernel_SetFreeSurface! = SetFreeSurface!(device)
    ...
    for iter=1:itmax
        # if options.free_surface
            # wait( kernel_SetFreeSurface!(V, P, η, Δ; ndrange=(ncx+2,ncy+2)) )
        # end
        wait( kernel_StrainRates!(ε̇, ∇v, V, P, η, ∂, Δ, options; ndrange=(ncx+2,ncy+2)) )
        wait( kernel_Stress!(τ, η, ε̇; ndrange=(ncx+2,ncy+2)) )
        wait( kernel_Residuals!(R, τ, P, ∂, b, Δ, options,            ε̇, η            ; ndrange=(ncx+2,ncy+2)) )
    ...
    ax1 = Axis(f[1, 2], title = "Surface Vx", xlabel = "x [km]", ylabel = "Vx [m/s]")
    lines!(ax1, X.v.x[2:end-1,end][:], to_host(V.x.v[2:end-1,end][:]))
