...
function Stokes3D(n, ::Type{DAT}; device) where DAT
    ...
    x0    = device_array(DAT, device, ncx+1, ncy+2, ncz+2); fill!(x0,  DAT(0.))
    y0    = device_array(DAT, device, ncx+2, ncy+1, ncz+2); fill!(y0,  DAT(0.))
    z0    = device_array(DAT, device, ncx+2, ncy+2, ncz+1); fill!(z0,  DAT(0.))
    p0    = device_array(DAT, device, ncx+2, ncy+2, ncz+2); fill!(p0,  DAT(0.))

    x1    = device_array(DAT, device, ncx+1, ncy+2, ncz+2); fill!(x1,  DAT(0.))
    y1    = device_array(DAT, device, ncx+2, ncy+1, ncz+2); fill!(y1,  DAT(0.))
    z1    = device_array(DAT, device, ncx+2, ncy+2, ncz+1); fill!(z1,  DAT(0.))
    p1    = device_array(DAT, device, ncx+2, ncy+2, ncz+2); fill!(p1,  DAT(0.))
    ...
    # Steps
    hx[:,2:end-1,2:end-1]    .= 1.0 ./av2yz_arit(ηv2)*4
    hy[2:end-1,:,2:end-1]    .= 1.0 ./av2zx_arit(ηv2)*4
    hz[2:end-1,2:end-1,:]    .= 1.0 ./av2xy_arit(ηv2)*4
    hx[:,[1, end],:] .=  hx[:,[2, end-1],:];  hx[:,:,[1, end]] .=  hx[:,:,[2, end-1]]
    hy[[1, end],:,:] .=  hy[[2, end-1],:,:];  hy[:,:,[1, end]] .=  hy[:,:,[2, end-1]]
    hz[[1, end],:,:] .=  hz[[2, end-1],:,:];  hz[:,[1, end],:] .=  hz[:,[2, end-1],:];
    hp    .= 1.0  
    ...
    for iter=1:niter
        ...
        if mod(iter, nout) == 0 
            x0[:,2:end-1,2:end-1]  .=  Vx[:,2:end-1,2:end-1] .- hx[:,2:end-1,2:end-1].*dVxdτ  
            y0[2:end-1,:,2:end-1]  .=  Vy[2:end-1,:,2:end-1] .- hy[2:end-1,:,2:end-1].*dVydτ
            z0[2:end-1,2:end-1,:]  .=  Vz[2:end-1,2:end-1,:] .- hz[2:end-1,2:end-1,:].*dVzdτ
            p0[2:end-1,2:end-1,2:end-1] .=  P[2:end-1,2:end-1,2:end-1]   .- hp[2:end-1,2:end-1,2:end-1].*dPdτ
        end
        ...
        if mod(iter, nout) == 0 
            x1[:,2:end-1,2:end-1]  .=  Vx[:,2:end-1,2:end-1] .+ hx[:,2:end-1,2:end-1].*dVxdτ  
            y1[2:end-1,:,2:end-1]  .=  Vy[2:end-1,:,2:end-1] .+ hy[2:end-1,:,2:end-1].*dVydτ
            z1[2:end-1,2:end-1,:]  .=  Vz[2:end-1,2:end-1,:] .+ hz[2:end-1,2:end-1,:].*dVzdτ
            p1[2:end-1,2:end-1,2:end-1] .=  P[2:end-1,2:end-1,2:end-1]  .+ hp[2:end-1,2:end-1,2:end-1].*dPdτ
        end
