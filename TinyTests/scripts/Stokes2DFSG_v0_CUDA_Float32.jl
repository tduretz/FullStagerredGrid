...
using TinyKernels, Printf
import LinearAlgebra: norm
import Statistics: mean
@views av1D(x) = 0.5f0.*(x[2:end] .+ x[1:end-1])
...
@tiny function StrainRates!(ε̇, ∇v, V, Δ)
    ...
    @inbounds if isin(∇v.x)
        ...
        ∇v.x[ix,iy]   = ∂Vx∂x + ∂Vy∂y
        ε̇.xx.x[ix,iy] = ∂Vx∂x - 1f0/3f0*(∂Vx∂x + ∂Vy∂y)
        ε̇.yy.x[ix,iy] = ∂Vy∂y - 1f0/3f0*(∂Vx∂x + ∂Vy∂y)
        ε̇.xy.x[ix,iy] = 1f0/2f0*(∂Vx∂y + ∂Vy∂x)
    end
    @inbounds if isin(∇v.y)
        ...
        ∇v.y[ix,iy]   = ∂Vx∂x + ∂Vy∂y
        ε̇.xx.y[ix,iy] = ∂Vx∂x - 1f0/3f0*(∂Vx∂x + ∂Vy∂y)
        ε̇.yy.y[ix,iy] = ∂Vy∂y - 1f0/3f0*(∂Vx∂x + ∂Vy∂y)
        ε̇.xy.y[ix,iy] = 1f0/2f0*(∂Vx∂y + ∂Vy∂x)
    end
    return
end
...
@tiny function Stress!(τ, η, ε̇)
    ...
    @inbounds if isin(τ.xx.x)
        τ.xx.x[ix,iy] = 2f0 * η.x[ix,iy] * ε̇.xx.x[ix,iy]
        τ.yy.x[ix,iy] = 2f0 * η.x[ix,iy] * ε̇.yy.x[ix,iy]
        τ.xy.x[ix,iy] = 2f0 * η.x[ix,iy] * ε̇.xy.x[ix,iy]
    end
    @inbounds if isin(τ.xx.y)
        τ.xx.y[ix,iy] = 2f0 * η.y[ix,iy] * ε̇.xx.y[ix,iy]
        τ.yy.y[ix,iy] = 2f0 * η.y[ix,iy] * ε̇.yy.y[ix,iy]
        τ.xy.y[ix,iy] = 2f0 * η.y[ix,iy] * ε̇.xy.y[ix,iy]
    end
    return
end
...
function main(::Type{DAT}; device) where DAT
    ...
    ε̇bg      = -1f0
    rad      = 0.5f0
    ϵ        = 5f-4      # nonlinear tolerence
    iterMax  = 5f4     # max number of iters
    ...
    ηx[xv.x.^2 .+ (xc.y').^2 .< rad^2] .= 1.1f0
    ηy = ones(Float32, ncx, ncy+1)
    ηy[xc.x.^2 .+ (xv.y').^2 .< rad^2] .= 1.1f0
    η.x   .= to_device( ηx )
    η.y   .= to_device( ηy )
    PS     = 1
    ...
    θ        = 6f0/ncx
    ΔτV      = 0.3f0*cfl*max(Δ...)^2 ./ maximum(ηx)
    ΔτP      = 1f0*cfl*minimum(ηx)*min(Δ...)/2f0
