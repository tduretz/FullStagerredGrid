...
@views function Stokes2S_FSG()
    ...
    # Density
    ρ_1  .= 1.0; ρ_2  .= 1.0
    ρ_1[xv2_1.^2 .+ yv2_1.^2 .< rad] .= 2.
    ρ_2[xv2_2[2:end-1,2:end-1].^2 .+ yv2_2[2:end-1,2:end-1].^2 .< rad] .= 2.
    # Smooth Viscosity
    ...
    while (err>ε && iter<=iterMax)
        ...
        Ry_1[2:end-1,2:end-1] .= diff(τyy_2[2:end-1,:], dims=2)./Δy .+ diff(τxy_1[:,2:end-1], dims=1)./Δx .-  diff(P_2[2:end-1,:], dims=2)./Δy .+ ρ_1[2:end-1,2:end-1].*g
        Ry_2                  .= diff(τyy_1,            dims=2)./Δy .+ diff(τxy_2,            dims=1)./Δx .-  diff(P_1           , dims=2)./Δy .+ ρ_2.*g
        Rp_1                  .= .-∇v_1
        Rp_2                  .= .-∇v_2
